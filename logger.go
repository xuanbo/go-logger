...
type Logger struct {
	...
	wrapperLevel int // wrapper level
}
...
// Attach start attach a logger adapter
...
// Detach start attach a logger adapter
// param : adapterName console | file | database | ...
// return : error
...
// SetAsync set logger synchronous false
// params : sync bool
...
// SetWrapperLevel wrapper level
func (logger *Logger) SetWrapperLevel(wrapperLevel int) {
	logger.wrapperLevel = wrapperLevel
}
...
func (logger *Logger) Writer(level int, msg string) error {
	funcName := "null"
	pc, file, line, ok := runtime.Caller(logger.wrapperLevel + 2)
	if !ok {
		file = "null"
		line = 0
	} else {
...
// writeToOutputs sync write message to loggerOutputs
